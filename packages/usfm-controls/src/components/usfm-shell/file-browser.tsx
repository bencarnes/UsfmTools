...
export function FileBrowser({ files, activeFileId, onOpenFile, loading, label }: FileBrowserProps) {
  return (
    <div className="flex h-full min-h-0 flex-col" data-testid="usfm-shell-file-browser">
      <div className="border-b border-gray-200 bg-gray-50 px-3 py-1.5 text-xs font-semibold uppercase tracking-wide text-gray-700">
        {label}
      </div>
      <div className="min-h-0 flex-1 overflow-auto">
        {loading ? (
          ...
          <div className="px-3 py-2 text-sm text-gray-500">No files in folder</div>
        ) : (
          <ul className="py-1">
            {files.map((f) => {
                ...
                <li key={f.id}>
                  <button
                    ...
                    className={`block w-full truncate px-3 py-1 text-left text-sm ${
                      active ? "bg-blue-100 text-blue-900" : "text-gray-800 hover:bg-gray-100"
                    }`}
