...
export function ErrorsPanel({ activeFileName, diagnostics, onSelectDiagnostic, loading }: ErrorsPanelProps) {
  return (
    <div className="flex h-full min-h-0 flex-col" data-testid="usfm-shell-errors-panel">
      <div className="border-b border-gray-200 bg-gray-50 px-3 py-1 text-xs uppercase tracking-wide text-gray-600">
      ...
      <div className="min-h-0 flex-1 overflow-auto">
        {loading ? (
          <div className="px-3 py-2 text-sm text-gray-500">Checking…</div>
        ) : !activeFileName ? (
          <div className="px-3 py-2 text-sm text-gray-500">No file open</div>
        ) : diagnostics.length === 0 ? (
          <div className="px-3 py-2 text-sm text-gray-500">No errors</div>
        ) : (
          <ul className="py-1">
            {diagnostics.map((d, i) => (
              <li key={`${d.range.start.line}:${d.range.start.column}:${i}`}>
                ...
                >
                  <span className="mr-2 inline-block min-w-[3.5rem] font-mono text-xs text-gray-500">
