...
Preview HTML regeneration is debounced (1.5 s) in split mode. Scroll sync is paused during typing and resumes after 500 ms idle. Workspace model updates are debounced (500 ms) while dirty state is marked immediately; save flushes the editor buffer first. Syntax highlighting classifies only the visible viewport (plus a few lines of margin) and maps existing decorations through edits instead of rebuilding the full document on every pause. Remaining per-keystroke cost is dominated by full-document validation inside CodeMirror.
